...
static int blob_write(lua_State* L)
{
   ...
   rc = sqlite3_blob_write(sql_blob, p, l, bofs);
...
static int blob_read(lua_State* L)
{
   ...
   do {
      ...
      rc =  sqlite3_blob_read(sql_blob, p, rlen, bofs);
      if (rc != SQLITE_OK) {
         lua_settop(L,2);
         return pusherr(L, rc, sqlite3_errmsg(blob->conn->sql_conn));
      }
...
static int conn_blobzero(lua_State* L)
{
   ...
   rc = sqlite3_bind_zeroblob(vm, 1, sz);
   if (rc != SQLITE_OK) {
      errmsg = sqlite3_errmsg(conn->sql_conn);
...
static int create_connection(lua_State *L, int env, sqlite3 *sql_conn)
{
   ...
   /* set a default busy timeout */
   sqlite3_busy_timeout(sql_conn, 200);
...
{
   lua_pushnumber(L, (lua_Number)sqlite3_memory_used());
   lua_pushnumber(L, (lua_Number)sqlite3_memory_highwater(0));
   return 2;
}
